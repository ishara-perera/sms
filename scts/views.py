...
from scts.serializers import StudentCreateSerializer, StudentSerializer, ClassroomSerializer
...
# @api_view(['PUT'])
# def update_delivery_partner(request, pk):
#     try:
#         # Retrieve the Delivery Partner instance from the database
#         instance = DeliveryPartner.objects.get(pk=pk)
#         serializer = DeliveryPartnerSerializer(instance=instance, data=request.data, partial=True)
#
#         if serializer.is_valid():
#             serializer.save()
...
@api_view(['GET'])
def get_classrooms_all(request):
    try:
